...
void cGame::initialise(SDL_Window* theSDLWND, SDL_Renderer* theRenderer)
{
	...
	SDL_GameController *controller = nullptr; //pointer
		//
	for (int i = 0; i < SDL_NumJoysticks(); i++) // checks for controller
	{
	...
	{
		theFontMgr->addFont(fontList[fonts], fontsToUse[fonts], 72);
	}
	gameTextList = { "Battlegrounds",  "Score : ","Greer Galloway ", "move - arrowkeys/left and right on dpad", "shoot - spacebar/A"};
	...
	theTextureMgr->addTexture("name", theFontMgr->getFont("spaceAge")->createTextTexture(theRenderer, gameTextList[2], textType::solid, { 0, 255, 0, 255 }, { 0, 0, 0, 0 }));
	theTextureMgr->addTexture("controls", theFontMgr->getFont("spaceAge")->createTextTexture(theRenderer, gameTextList[3], textType::solid, { 0, 255, 0, 255 }, { 0, 0, 0, 0 }));
	theTextureMgr->addTexture("controls2", theFontMgr->getFont("spaceAge")->createTextTexture(theRenderer, gameTextList[4], textType::solid, { 0, 255, 0, 255 }, { 0, 0, 0, 0 }));
	...
	/*for (int astro = 0; astro < 5; astro++)
	{
		...
		theAsteroids[astro]->setSpritePos({ (50 * (astro+1) + (50 * (astro + 1)+(50*astro))), -75 });
...
void cGame::render(SDL_Window* theSDLWND, SDL_Renderer* theRenderer)
{
	...
	tempTextTexture->renderTexture(theRenderer, tempTextTexture->getTexture(), &tempTextTexture->getTextureRect(), &pos, scale);
	tempTextTexture = theTextureMgr->getTexture("name");
	...
	tempTextTexture->renderTexture(theRenderer, tempTextTexture->getTexture(), &tempTextTexture->getTextureRect(), &pos, scale);
	tempTextTexture = theTextureMgr->getTexture("controls2");
	pos = { 50, 150, tempTextTexture->getTextureRect().w, tempTextTexture->getTextureRect().h };
	tempTextTexture->renderTexture(theRenderer, tempTextTexture->getTexture(), &tempTextTexture->getTextureRect(), &pos, scale);
	// Render updated score value
	if (updateScore)
	{
		theTextureMgr->deleteTexture("theScore");
		theTextureMgr->addTexture("theScore", theFontMgr->getFont("spaceAge")->createTextTexture(theRenderer, strScore.c_str(), textType::solid, { 255, 255, 255, 255 }, { 255, 255, 255, 255 }));
		updateScore = false;
	}
	...
	*/
	...
	if (frames % 360 == 0)
	{
		...
		int count = 1;
			...
			{
				theAsteroids.push_back(new cAsteroid);
				theAsteroids[astro]->setSpritePos({(150 * count ), -75 }); //spaced out the spawning to better suit the sprites
				theAsteroids[astro]->setSpriteTranslation({ 0, -100 }); //changed velocity to better suit playable space size
				int randAsteroid = AsteroidTextDis(gen);
				theAsteroids[astro]->setTexture(theTextureMgr->getTexture(textureName[randAsteroid]));
				theAsteroids[astro]->setSpriteDimensions(theTextureMgr->getTexture(textureName[randAsteroid])->getTWidth(), theTextureMgr->getTexture(textureName[randAsteroid])->getTHeight());
		...
		switch (event.type)
		{
			...
			case SDL_KEYDOWN:
				...
				case SDL_CONTROLLER_BUTTON_A:
				{
					...
					theBullets[numBullets]->setSpriteTranslation({ 180, 30 });
					...
					theBullets[numBullets]->setSpriteRotAngle(theRocket.getSpriteRotAngle() - 90.0f);
				...
				case SDLK_SPACE:
				{
					theBullets.push_back(new cBullet);
					int numBullets = theBullets.size() - 1;
					theBullets[numBullets]->setSpritePos({ theRocket.getBoundingRect().x + theRocket.getSpriteCentre().x -100, theRocket.getBoundingRect().y + theRocket.getSpriteCentre().y });
					theBullets[numBullets]->setSpriteTranslation({ 180, 30 });
					theBullets[numBullets]->setTexture(theTextureMgr->getTexture("photon"));
					theBullets[numBullets]->setSpriteDimensions(theTextureMgr->getTexture("photon")->getTWidth(), theTextureMgr->getTexture("photon")->getTHeight());
					theBullets[numBullets]->setBulletVelocity(50);
